...
export default function Dashboard() {
  ...
  return <div className="app-shell">
    <aside className="app-sidebar" aria-label="Primary navigation">
      ...
      <nav className="side-nav"><Link className="side-nav__item side-nav__item--active" to="/dashboard"><LayoutDashboard /> Overview</Link><a className="side-nav__item" href="#new-request"><Plus /> New request</a><a className="side-nav__item" href="#my-bookings"><CalendarDays /> My bookings</a>{Boolean(staff?.isAdmin) && <Link className="side-nav__item" to="/admin"><ShieldCheck /> Approval centre</Link>}</nav>
    ...
    <main className="app-main">
      <header className="workspace-header"><div><p className="eyebrow">Managing Director’s Office</p><h1>Boardroom operations</h1></div><div className="header-actions">{Boolean(staff?.isAdmin) && <Link to="/admin" className="quiet-link">Review requests <ChevronRight /></Link>}<button className="icon-button" onClick={() => setLogoutOpen(true)} aria-label="Log out"><LogOut /></button></div></header>
    ...
    <ConfirmationDialog open={logoutOpen} title="Log out of Boardroom" description="You will need to sign in again to access booking information." confirmLabel="Log out" confirmVariant="danger" cancelLabel="Stay signed in" onConfirm={logout} onCancel={() => setLogoutOpen(false)} />
  </div>
}
